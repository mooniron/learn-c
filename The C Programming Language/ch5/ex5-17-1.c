...
/*
exercise 5-17:
add a field-searching capability, so sorting may be done on fields within lines, each field sorted
 according to an independent set of options.

outline:
    1. if -n is on, sort lines nummerically first
    2. then find the line number of the first number line, and first letter line.
    3. qsort the letter lines
...
/*
exercise 5-16:
add -d ("directory order") option, which makes comparisons only on letters, numbers and blanks.
*/
...
int reverse = 0;
/*  fold lower and upper case together  */
int nocase = 0;
        ...
        while ((c = *++argv[0]) != '\0') {
            switch(c) {
                case 'n':
                    numeric = 1;
                    break;
                /*  add in exercise 5-14  */
                case 'r':
                    reverse = 1;
                    break;
                /*  add in exercise 5-11  */
                case 'f':
                    nocase = 1;
                    break;
                default:
...
int strcmpr(char *s1, char *s2) {
    ...
    /*  exercise 5-15  */
    while ((c1 = nocase ? lowcase(*p1) : *p1 ) == (c2 = nocase ? lowcase(*p2) : *p2)) {
        if (*p1 == '\0') {
            return 0;
            }
        p1++;
...
/*  lowcase: lower case of input string  */
char lowcase(char s) {
    if (s >= 'A' && s <= 'Z') {
        s += 'a' - 'A';
        }   //end if
