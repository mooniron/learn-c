...
/*    printf("inquote = %d, inquotecom = %d, incomment = %d, incomquote = %d\n", \
    ...
    for (n = 0; n < NKEYS; n++) {
        if (keytab[n].count > 0) {
            printf("keytab.count = %4d %s\n", keytab[n].count, keytab[n].word);
            }   //end if
        if (keytab[n].quoten > 0) {
            printf("keytab.quoten = %4d %s\n", keytab[n].quoten, keytab[n].word);
            }
        if (keytab[n].commentn > 0) {
            printf("keytab.commentn = %4d %s\n", keytab[n].commentn, keytab[n].word);
            }
        }   //end for
